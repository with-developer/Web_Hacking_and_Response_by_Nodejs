...
router.post("/form_notice", function (req, res, next) {
    ...
    if (id === undefined) {
        ...
        connection.query("insert into notice (title, content, author) VALUES ?",
            [[[title, content, id]]],
...
router.post("/comment_form_notice/:idx", function (req, res, next) {
    ...
    if (id === undefined) {
        ...
        connection.query("insert into notice_comment (board_id, content, author) VALUES ?",
            [[[board_id, content, id]]],
            function (err, results, fields) {
                if (err) console.log(err);
...
router.get('/read/:idx', function (req, res, next) { // board/read/idx숫자 형식으로 받을거
    ...
    connection.query(sql, [idx], function (err, results) {  // 한개의 글만조회하기때문에 마지막idx에 매개변수를 받는다
        ...
        connection.query(comment, [idx], function (err, comment_results) {
            ...
            res.render('read_notice', { title: '글 상세보기', results: results[0], user: req.session.name, comment_results: comment_results, length: comment_results.length - 1 } ); // 첫번째행 한개의데이터만 랜더링 요청    
        });
...
router.get('/page/:page', function (req, res, next) { // 게시글 리스트에 :page가 추가된것임
    ...
    connection.query(sql, function (err, rows) {
        ...
        console.log(rows)
        res.render('page_notice', { rows: rows, page: page, length: rows.length - 1, page_num: 10, pass: true, moment, user: req.session.name });
